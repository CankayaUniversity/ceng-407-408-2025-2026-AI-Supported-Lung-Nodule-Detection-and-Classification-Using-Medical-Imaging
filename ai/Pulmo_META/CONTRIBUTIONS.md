# Pulmo – Project Log (Superproject)

## Core model (single source of truth)
- Location: `ai/Pulmo` (git submodule)
- Maintainer: Furkan (fc63)
- Note: The submodule directory is treated as read-only in this repository workflow.

## Experiments / Proposals (do not modify core model)
### Attention Gate experiment (Orkun)
- Origin: commit `dfd86b8` (ported from legacy `lung25demo` work)
- Status: experiment only (not integrated into `ai/Pulmo`)
